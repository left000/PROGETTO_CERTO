...
void saveCertificatiFile(ListaCertificati lista, const char *nomeFile)
{
  ...
  while (corrente != NULL)
  {
    ...
    fprintf(fp, "\nCertificato ID: %d\n", c.id_certificato);
    fprintf(fp, "Numero seriale: %s\n", c.numero_seriale);
    fprintf(fp, "Data emissione: %02d/%02d/%04d\n",
            c.data_emissione.giorno, c.data_emissione.mese, c.data_emissione.anno);
    fprintf(fp, "Stato: %s\n", c.stato);
    fprintf(fp, "Camera commercio: %s\n", c.camera_commercio);

    if (az != NULL)
    {
      fprintf(fp, "Azienda: %s (P.IVA: %s) | %s, %s\n",
              az->ragione_sociale, az->partita_iva, az->indirizzo, az->nazione);
    }
    else
    {
      fprintf(fp, "Azienda: NON TROVATA\n");
    }

    if (pr != NULL)
    {
      fprintf(fp, "Prodotto: %s | Peso: %.2f kg | Valore: %.2f € | Codice TARIC: %s\n",
              pr->descrizione, pr->peso_kg, pr->valore_euro, pr->codice_taric);
    }
    else
    {
      fprintf(fp, "Prodotto: NON TROVATO\n");
    }

    if (dest != NULL)
    {
      fprintf(fp, "Destinazione: %s | Porto: %s | Mezzo: %s\n",
              dest->paese, dest->porto_arrivo, dest->mezzo_trasporto);
    }
    else
    {
      fprintf(fp, "Destinazione: NON TROVATA\n");
    }
  ...
  fclose(fp);
  printf("Dati certificati salvati correttamente in %s\n", nomeFile);
}
// void saveCertificatiFile(ListaCertificati lista, const char *nomeFile)
// {
//   FILE *fp = fopen(nomeFile, "w");
